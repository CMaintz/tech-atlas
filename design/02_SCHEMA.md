...
| E11 | **Semantic vectors out of date.** `supabase/seed/term-vectors.json` has a vector for every Term (and none for removed ones) and was built with the current model settings; re-run `npm run embed` (A52, A62). | |
...
| W8 | **Semantic vector embeds older text.** A Term's name, aliases, summary or plain facet changed since `npm run embed` (A52, A62). | Its search-by-meaning match is slightly stale until re-embedded. |
